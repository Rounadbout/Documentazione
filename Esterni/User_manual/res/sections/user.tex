...
\subsubsubsection*{Usage}
Suppose you have deployed a function called \texttt{mul} some time ago and now you want to change its behaviour. In particular, you want to update its source code, which is contained in a file \texttt{mul.js} in the current directory. To do that, type from your terminal window:
\begin{center}
	\code{etherless edit mul -s mul.js}
\end{center}
Where \texttt{path} is thoroughly described in section \ref{deploy}.
